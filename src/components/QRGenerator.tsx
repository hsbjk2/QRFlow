...
export default function QRGenerator({ onAddHistory }: QRGeneratorProps) {
  ...
  const [socialUsername, setSocialUsername] = useState('qrflow_app');
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="w-10 h-10 border border-slate-250 dark:border-slate-700 bg-transparent rounded-xl cursor-pointer overflow-hidden p-0"
                  />
                  <input
                    type="text"
                    value={fgColor}
                    onChange={(e) => setFgColor(e.target.value)}
                    className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800/40 border border-slate-200/50 dark:border-slate-800 rounded-xl text-xs font-mono font-bold text-slate-700 dark:text-slate-200 focus:ring-1 focus:ring-indigo-500 focus:outline-none"
                  />
                </div>
              </div>
